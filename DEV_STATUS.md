...
## Sprint 13 — Mobile, Morning Brief, Curiosity Engine (13 июня 2026)

### Backend
- `GET /api/chat/morning-brief` — AIR4 говорит первым если нет сообщений за сегодня
- Кэш по дате (новый день = новый бриф), TTL 1 час внутри дня
- Тренировка читается явно из таблицы `workouts` с расчётом дней назад
- Interview вопрос вплетается в Morning Brief раз в 3 дня через существующий `interviewer` сервис
- `unified_extractor.py` — 4 LLM экстрактора объединены в один вызов (убраны 429)

### Frontend
- `App.tsx` — редирект на FullscreenChat если ширина < 768px
- `FullscreenChat.tsx` — мобильная версия: скрыта левая панель, упрощена шапка, инпут внизу
- Morning Brief отображается как последнее сообщение с подписью "Доброе утро"

### Инфраструктура
- Tailscale настроен — AIR4 доступен с телефона по `100.115.122.70:3000`
- Backend и frontend запускаются с `--host 0.0.0.0` для внешних подключений
- `frontend/` папка удалена (была мёртвым Next.js прототипом)
...
1. **Жить с продуктом** — фиксировать "чёрт… он прав" моменты
2. **Итерировать промпты** Morning Brief и Recommendation на реальных данных
3. **Follow-up Engine** — AIR4 возвращается к важным событиям
4. **Observation Engine** — улучшать качество на реальных данных
5. **Загружать выписки регулярно** — каждые 10 числа
