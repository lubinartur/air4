...
from services.prompts import OVERVIEW_ADVISOR_FORMAT
...
def _build_llm_prompt(context: dict[str, Any], signals: list[dict[str, Any]]) -> str:
    return (
        ...
        "Текст попадает в карточку Open Loops на экране Обзор.\n\n"
        "Сигналы rule layer:\n"
        ...
        "- Нельзя предложить конкретный следующий шаг на сегодня\n\n"
        "ОТПРАВЬ если:\n"
        "- Есть неочевидная связь между двумя сферами\n"
        "- Есть конкретная цифра которая удивит\n"
        "- Есть паттерн который повторяется 2+ раза\n"
        "- Можешь довести до конкретной рекомендации и действия на сегодня\n\n"
        "ШАГ 2 — ФОРМУЛИРОВКА:\n"
        f"{OVERVIEW_ADVISOR_FORMAT}\n"
        "title — короткий заголовок (до 8 слов), отражает суть или действие.\n"
        "body — полный текст по структуре выше (4 предложения).\n"
        "Язык уверенности в первом предложении зависит от confidence:\n"
        '- < 0.5: "Начинает выглядеть как..."\n'
        '- 0.5-0.7: "Похоже что..."\n'
        '- > 0.7: "Судя по последним N случаям..."\n\n'
        "Запрещено: выводы без цифр, морализаторство, заканчивать только наблюдением.\n\n"
