...
def _build_llm_prompt(context: dict[str, Any], signals: list[dict[str, Any]]) -> str:
    return (
        "Ты формулируешь наблюдения для AIR4 — личного советника.\n\n"
        "Сигналы rule layer (уже подтверждены данными):\n"
        f"{json.dumps(signals, ensure_ascii=False, indent=2)}\n\n"
        "Контекст для формулировки:\n"
        f"{json.dumps(context, ensure_ascii=False, indent=2)}\n\n"
        "Сгенерируй 1–3 наблюдения на русском языке.\n"
        "Стиль: прямой, с цифрами, без воды. Максимум 3 предложения на наблюдение:\n"
        "1) факт с цифрой; 2) связь или паттерн; 3) действие ИЛИ вопрос (не оба).\n"
        "Запрещено: «тебе следует», «попробуй», «рекомендую», общие советы без цифр.\n"
        "Наблюдение — гипотеза, не утверждённый факт.\n\n"
        "Верни ТОЛЬКО JSON-массив (без markdown). Каждый объект:\n"
        "{\n"
        ...
        "}\n"
        "Поле evidence_refs не включай — будет пустым массивом.\n"
        "Если сигналов недостаточно для качественного текста — верни []."
    )
